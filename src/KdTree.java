...
public class KdTree {
  ...
  private boolean find(Node node, Point2D p) {
    ...
    //initialize to true as we always start on an X-level
    boolean isX = true;
  ...
  public Iterable<Point2D> range(RectHV rect) {
    ...
    List<Point2D> myList = new ArrayList<>();
  ...
  private Node findNearest(Point2D p, Node next, Node nearest) {
    ...
    if (next.isXLevel) {
      if (p.x() < next.p.x()) {
        // go left
        if (next.lb != null && next.rt != null
            && p.distanceTo(nearest.p) < p.distanceTo(new Point2D(next.rt.rect.xmin(), p.y()))) {
        ...
        // go right
        if (next.lb != null && next.rt != null
            && p.distanceTo(nearest.p) < p.distanceTo(new Point2D(next.lb.rect.xmax(), p.y()))) {
      ...
      if (p.y() < next.p.y()) {
        // go left
        if (next.lb != null && next.rt != null
            && p.distanceTo(nearest.p) < p.distanceTo(new Point2D(p.x(), next.rt.rect.ymin()))) {
          // no need to go down right tree
          nearest = findNearest(p, next.lb, nearest);
        } else {
          // check both trees
        ...
        // go right
        if (next.lb != null && next.rt != null
            && p.distanceTo(nearest.p) < p.distanceTo(new Point2D(p.x(), next.lb.rect.ymax()))) {
          // no need to go down right tree
          nearest = findNearest(p, next.rt, nearest);
        } else {
          // check both trees
